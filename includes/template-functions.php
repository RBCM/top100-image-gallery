    ...
    <div class="image-gallery-area">
      ...
      <?php
  		foreach ( $attachment_ids as $attachment_id ) {
  			...
  			$image_url = wp_get_attachment_image_src( $attachment_id, 'gallery-thumb');
  			
  			$image_html = '<img src="'.$image_url[0].'" alt="'.get_post_meta( $attachment_id, '_wp_attachment_image_alt', true ).'">';
  			...
  			$rel = easy_image_gallery_count_images() > 1 ? 'rel="fancybox[group]"' : 'rel="fancybox"';
  
        $html = sprintf( '<li><a %s href="%s" class="%s" title="%s"><i class="icon-view"></i><span class="overlay"></span>%s</a></li>', $rel, $image_link, $image_class, $image_caption, $image_html );
  
  			echo apply_filters( 'easy_image_gallery_html', $html, $rel, $image_link, $image_class, $image_caption, $image_html, $attachment_id, $post->ID );
  		}
  ?>
      </ul>
    </div>
